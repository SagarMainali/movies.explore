     ...
     return (
          isLoading && !data
               ...
               <NavLink to={`/${trending.media_type}/${trending.id}`}>
                    <div className='relative overflow-hidden h-[99vh] max-h-[700px]'>
                         <LazyImage
                              src={trending.backdrop_path ? image_baseUrl + trending.backdrop_path : '/no-poster.png'}
                              alt="banner-image"
                              className={`object-center h-full w-full ${trending.backdrop_path ? 'object-cover' : 'object-contain'}`}
                         />
                         ...
                         <GradientOverlay>
                              <div className='flex flex-col gap-4 md:w-[60vw] w-full'>
                                   ...
                                   <div className='flex gap-3'>
                                        <h3 className='px-2 text-center uppercase text-[12px] font-bold bg-slate-300 text-primary-dark rounded-md'>{trending.media_type}</h3>
                                        <h3 className='px-2 text-center uppercase text-[12px] font-bold bg-slate-300 text-primary-dark rounded-md'>{trending.original_language}</h3>
                                        {
                                             trending.release_date && <h3 className='w-[75px] text-center uppercase text-[12px] font-bold bg-slate-300 text-primary-dark rounded-md'>{trending.release_date}</h3>
                                        }
                                        <h3 className='px-2 text-center uppercase text-[12px] font-bold bg-slate-300 text-primary-dark rounded-md flex gap-1 items-center'>
